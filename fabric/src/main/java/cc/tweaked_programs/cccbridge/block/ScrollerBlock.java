...
public class ScrollerBlock extends FacingBlock implements BlockEntityProvider, Waterloggable {
    public ScrollerBlock() {
        ...
        setDefaultState(this.stateManager.getDefaultState().with(Properties.FACING, Direction.NORTH).with(Properties.WATERLOGGED, false));
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        return this.getDefaultState().with(Properties.FACING, ctx.getSide()).with(Properties.WATERLOGGED,ctx.getWorld().getFluidState(ctx.getBlockPos()).getFluid() == Fluids.WATER);
    }
}
